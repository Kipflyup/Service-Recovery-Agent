#!/usr/bin/env python3
"""Diagnose the latest service traceback with deterministic fault rules."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="读取最新 Traceback，构建 CodeContext，并输出确定性的 FaultDiagnosis。"
    )
    parser.add_argument(
        "--dotenv",
    ...
    parser.add_argument(
        ...
        help="项目根目录，用于过滤项目内栈帧、扫描调用方和追踪变量来源。",
    )
    parser.add_argument(
        "--context-lines",
        type=int,
...
def main() -> int:
    ...
    if args.json:
        ...
        print(format_fault_diagnosis_report(diagnosis))
